...
def process_revision_directives(
    ...
    """ """
    skipped = skip_empty_migrations(directives)
    if not skipped:
        set_revision_id_to_sequential_int(context, directives)


def skip_empty_migrations(directives: list[MigrationScript]) -> bool:
    """ """
...
def run_migrations_offline(url: MultiHostUrl) -> None:
    ...
    context.configure(
        ...
        # see DocString of 'include_object()' on why we need it and the custom 'version_table':
        include_object=include_object,
        version_table="alembic_version_sessions_service",
    )
